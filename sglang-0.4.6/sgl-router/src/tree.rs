...
impl Tree {
    ...
    */
    ...
    pub fn prefix_len_tenant_no_touch(&self, text: &str, tenant: &str) -> usize {
        ...
        while curr_idx < text_count {
            let first_char = match text.chars().nth(curr_idx) {
                Some(c) => c,
                None => break,
            };
            ...
            if let Some(child_ref) = curr.children.get(&first_char) {
                let matched_node = Arc::clone(child_ref.value());

                if !matched_node.tenant_last_access_time.contains_key(tenant) {
                    break;
                }

                let node_text_guard = matched_node.text.read().unwrap();
                let node_text = &*node_text_guard;
                let shared_count = shared_prefix_count(node_text, &curr_text);
                let node_text_count = node_text.chars().count();

                if shared_count == node_text_count {
                    curr_idx += shared_count;
                    prev = Arc::clone(&matched_node);
                } else {
                    curr_idx += shared_count;
