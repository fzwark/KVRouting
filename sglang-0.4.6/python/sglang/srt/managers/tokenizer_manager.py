...
def _extract_service_metrics(ts) -> dict:

    if isinstance(ts, dict):
        return ts


    if not isinstance(ts, TimeStats):
        return {}


    req_type = getattr(ts, "request_type", None)
    if hasattr(req_type, "value"):
        req_type = req_type.value
    elif callable(getattr(ts, "get_type", None)):
        _t = ts.get_type()
        req_type = getattr(_t, "value", _t)

    wait_q  = getattr(ts, "wait_queue_entry_time", None)
    fwd_in  = getattr(ts, "forward_entry_time", None)
    done    = getattr(ts, "completion_time", None)
    lb_in   = getattr(ts, "lb_entry_time", None)

    def ms(a, b):
        if a is None or b is None:
            return 0.0
        return max(0.0, (a - b) * 1e3)

    service_ms = ms(done, fwd_in)    
    queue_ms   = ms(fwd_in, wait_q)  

    return {
        "request_type": req_type,
        "service_time_ms": round(service_ms, 2),
        "queue_time_ms": round(queue_ms, 2),
        "start_time_s": wait_q or lb_in,
    }
...
class TokenizerManager:
    ...
    def _handle_batch_output(
        ...
        for i, rid in enumerate(recv_obj.rids):
            ...
            metrics = {}
            if hasattr(recv_obj, "time_stats"):
                ts_all = recv_obj.time_stats
                ts = ts_all[i] if isinstance(ts_all, list) else ts_all
                if ts:
                    metrics = _extract_service_metrics(ts)  
                 
                    meta_info["service_time_ms"] = metrics.get("service_time_ms")
                    meta_info["queue_time_ms"] = metrics.get("queue_time_ms")
            ...
            if metrics:
                state.extra_metrics = metrics
                out_dict["metrics"] = metrics
            elif getattr(state, "extra_metrics", None):
              
                out_dict["metrics"] = state.extra_metrics
