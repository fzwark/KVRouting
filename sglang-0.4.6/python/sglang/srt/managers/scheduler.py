...
from sglang.srt.mem_cache.radix_cache_lrt import RandomRadixCache
...
class Scheduler(
    ...
    def init_memory_pool_and_cache(self):
        ...
        if (
            ...
            elif server_args.enable_lrt:
                self.tree_cache = RandomRadixCache(
                    req_to_token_pool=self.req_to_token_pool,
                    token_to_kv_pool_allocator=self.token_to_kv_pool_allocator,
                    page_size=self.page_size,
    ...
    def event_loop_normal(self):
        ...
        while True:
            ...
            if batch:
                ...
                print("[log] something wrong in the loop.")
                self.check_memory()
                self.new_token_ratio = self.init_new_token_ratio
    ...
    def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
        ...
        for req in can_run_list:
            if getattr(req, "queue_time_end", None) is None:
                req.queue_time_end = time.time()
            if getattr(req, "service_start_time_s", None) is None:
                req.service_start_time_s = req.queue_time_end
