...
class SchedulerOutputProcessorMixin:
    ...
    def _make_time_stats(self, req, request_type: str):
        ts = {}
        q_s = getattr(req, "queue_time_start", None)
        s_s = getattr(req, "service_start_time_s", None)
        s_e = getattr(req, "service_end_time_s", None)
        if q_s is not None and s_s is not None:
            ts["queue_time_ms"] = max(0.0, (s_s - q_s) * 1000.0)
            ts["start_time_s"] = float(q_s)
        if s_s is not None:
            end_t = s_e if s_e is not None else time.time()
            ts["service_time_ms"] = max(0.0, (end_t - s_s) * 1000.0)
        ts["request_type"] = request_type  
        return ts
    ...
    def process_batch_result_decode(
        ...
        for req in batch.reqs:
            if req.finished() and getattr(req, "service_end_time_s", None) is None:
                req.service_end_time_s = now
    ...
    def stream_output_generation(
        ...
        for req in reqs:
            ...
            if (
                ...
                time_stats.append(self._make_time_stats(req, "generate"))
    ...
    def stream_output_embedding(self, reqs: List[Req]):
        ...
        for req in reqs:
            if req.finished():
                ...
                prompt_tokens.append(len(req.origin_input_ids))
                cached_tokens.append(req.cached_tokens)
                time_stats.append(self._make_time_stats(req, "embedding"))
        self.send_to_detokenizer.send_pyobj(
            BatchEmbeddingOut(
                rids, finished_reasons, embeddings, prompt_tokens, cached_tokens, time_stats
            )
        )
