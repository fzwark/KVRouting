...
async def generate_request(obj: GenerateReqInput, request: Request):
    ...
    if obj.stream:
            ...
            try:
                async for out in _global_state.tokenizer_manager.generate_request(
                    ...
                    if isinstance(out, dict) and "metrics" in out:
                        ...
                        yield b"data: " + orjson.dumps(
                            out["metrics"], option=orjson.OPT_NON_STR_KEYS
                        ) + b"\n\n"
        ...
        try:
            ...
            # return ret
            return _create_metric_response(ret)
        except ValueError as e:
            logger.error(f"Error: {e}")
...
def _create_metric_response(payload):
    """Wrap payload into ORJSONResponse and attach metric headers if present."""
    resp = ORJSONResponse(payload)
    if isinstance(payload, dict):
        m = payload.get("metrics") or {}
        if isinstance(m, dict) and m:
            resp.headers["X-SGLang-Service-Time-Ms"] = str(m.get("service_time_ms", 0.0))
            resp.headers["X-SGLang-Queue-Time-Ms"]   = str(m.get("queue_time_ms", 0.0))
